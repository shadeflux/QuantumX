...
-- OSOBNE OKNO TYLKO NA KEY SYSTEM
...
if KeyValid then
    ...
    Rayfield:Destroy()  -- niszczymy całe stare okno z key systemem
    task.wait(0.2)      -- małe opóźnienie, żeby uniknąć race condition
    LoadMainHub()
else
    if SavedKey then
    ...
    KeyTab:CreateInput({
        ...
        Callback = function(Token)
            if Token == "" then
                Rayfield:Notify({Title = "Błąd", Content = "Wklej klucz!", Duration = 4})
                return
            end
            ...
            if CheckKey(Token) then
                ...
                Rayfield:Destroy()  -- niszczymy całe okno z key systemem
                task.wait(0.2)
                LoadMainHub()
            else
                Rayfield:Notify({
...
function LoadMainHub()
    ...
    -- PLAYER MODS – wszystkie funkcje oprócz aimbota i fly/float
